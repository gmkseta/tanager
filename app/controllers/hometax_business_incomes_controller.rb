class HometaxBusinessIncomesController < ApplicationController
  ...
  def incomes
    ...
    @hometax_business_incomes = {
      total_income: @declare_user.hometax_business_incomes.sum(&:income_amount),
      business_incomes: results.keys.select{ |r| r.present? }.map {|key| { name: results[key].first.business_name, hometax_business_incomes: results[key].map { |v| { income_type: v.income_type, income_amount: v.income_amount } } } },
      other_incomes: results.keys.select{ |r| r.blank? }.map {|key| { name: results[key].first.business_name, hometax_business_incomes: results[key].map { |v| { income_type: v.income_type, income_amount: v.income_amount } } } },
    }
    render json: @hometax_business_incomes, status: :ok
  end
